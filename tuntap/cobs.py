...
def decode(encbytes_in: t.ByteString) -> t.ByteString:
    ...
    while True:
        try:
            ...
            logger.error(f"len in={len(encbytes_in)}; code_idx={code_idx}")
            raise(e)
    ...
    def process(self, new_data):
        ...
        while True:
            ...
            if self.state == "INIT":
                ...
                if self.q.empty():
                    logger.debug("DEFRAMER: FIND_EOF: fifo empty, waiting for more data.")
                    return None
                ...
                self.state = "INIT"
                try:
                    decode_data = decode(bytearray(self.data))
                except Exception as e:
                    logger.error(f"Error on decode: {str(e)}")
                    return None
                return decode_data
